...
export const Transactions = () => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        const res = await axios.get(`http://127.0.0.1:8000/api/transactions/${qrisTransactionId}`);
      ...
      <Dialog open={paymentConfirmOpen} onOpenChange={setPaymentConfirmOpen}>
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4 py-4">
            ...
            {paymentMethod === 'cash' && (
              <div className="space-y-2">
                ...
                />
                <div className="flex justify-between text-sm mt-2 p-2 bg-gray-100 rounded">
                  <span>Kembalian:</span>
                  <span className="font-bold text-primary">{formatRupiah(Math.max(0, cashAmount - calculateTotal()))}</span>
                </div>
              </div>
            )}
          ...
          <div className="flex gap-2">
            <Button variant="outline" className="flex-1" onClick={() => setPaymentConfirmOpen(false)}>Batal</Button>
            <Button
              className="flex-1 bg-primary"
                        disabled={!subPaymentMethod || (paymentMethod === 'cash' && cashAmount < calculateTotal())}
                        onClick={async () => {
                          setPaymentConfirmOpen(false);
                          if (paymentMethod === 'e-wallet' && subPaymentMethod === 'qris') {
                            await startQrisPayment();
                          } else {
                            processTransaction();
                          }
                        }}
