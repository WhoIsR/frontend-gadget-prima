...
export const Login = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 3. Simpan Token & User ke LocalStorage (biar diinget browser)
      ...
      // 5. Force reload biar aplikasi nge-refresh state auth-nya dari localStorage
      // (Ini cara paling gampang biar gak usah ngacak-ngacak useAuth dulu)
      window.location.reload(); 

    } catch (err: any) {
      // 6. Kalau gagal (401), ambil pesan error dari Laravel
      console.error("Login Error:", err);
      setError(err.response?.data?.message || 'Login gagal, cek koneksi atau password lu.');
    }
  };
