...
export const Settings = () => {
  ...
  const { users, expenses, refreshData } = useData();
  ...
  // --- 1. FETCH DATA (DIGANTI DENGAN CONTEXT) ---
  ...
  const handleUserSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingUser) {
        // Update User
        await axios.put(`${import.meta.env.VITE_API_URL}/users/${editingUser.id}`, userFormData);
        toast.success('Pengguna berhasil diperbarui');
      } else {
        // Create User
        await axios.post(`${import.meta.env.VITE_API_URL}/users`, userFormData);
  ...
  const handleUserDelete = async (id: string | number) => {
    if (window.confirm('Apakah Anda yakin ingin menghapus pengguna ini?')) {
      ...
      refresast.error('Gagal menghapus pengguna');
      }
    }
  };
  ...
  const handleExpenseSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingExpense) {
        await axios.put(`${import.meta.env.VITE_API_URL}/expenses/${editingExpense.id}`, expenseFormData);
        toast.success('Pengeluaran berhasil diperbarui');
      } else {
        await axios.post(`${import.meta.env.VITE_API_URL}/expenses`, expenseFormData);
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs defaultValue="users" className="space-y-4">
        <TabsList className="border-2 justify-start">
