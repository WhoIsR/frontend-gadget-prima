...
export const Layout = ({ children, currentPage, onPageChange }: LayoutProps) => {
  ...
  // --- BAGIAN INI YANG GUE BENERIN DIKIT ---
  const getMenuItems = () => {
    // 1. Ambil role user, jadiin huruf kecil semua biar aman
    const userRole = (user?.role || '').toLowerCase();

    const allItems = [
      // Dashboard untuk semua
      { id: 'dashboard', label: 'Dashboard', icon: LayoutDashboard, roles: ['admin', 'cashier', 'kasir', 'warehouse', 'gudang', 'owner', 'pemilik'] },
      
      // Produk: Admin & Gudang
      { id: 'products', label: 'Produk', icon: Package, roles: ['admin', 'warehouse', 'gudang'] },
      
      // Transaksi: Admin & Kasir
      { id: 'transactions', label: 'Transaksi', icon: ShoppingCart, roles: ['admin', 'cashier', 'kasir'] },
      
      // Laporan: Admin & Owner
      { id: 'reports', label: 'Laporan', icon: BarChart3, roles: ['admin', 'owner', 'pemilik'] },
      
      // Pengaturan: Admin doang
      { id: 'settings', label: 'Pengaturan', icon: Settings, roles: ['admin'] },
    ];

    // 2. Filter menu yang sesuai sama role user
  ...
  const handleLogout = () => {
    ...
    // Kita gak perlu navigate karena di App.tsx useAuth udah handle logout
  };

  const getRoleLabel = (role?: string) => {
    // Handle huruf besar/kecil biar labelnya tetep rapi
    switch (role?.toLowerCase()) {
      case 'admin': return 'Administrator';
      case 'cashier': return 'Kasir';
      case 'kasir': return 'Kasir'; // Jaga-jaga bahasa indo
      case 'warehouse': return 'Gudang';
      case 'gudang': return 'Gudang'; // Jaga-jaga bahasa indo
      case 'owner': return 'Pemilik';
      case 'pemilik': return 'Pemilik'; // Jaga-jaga bahasa indo
      default: return role;
    }
  };
