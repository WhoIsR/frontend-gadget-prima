...
import {
  ...
  BarChart3,
  // TrendingDown, // Tidak dipakai di render
  // ArrowUpRight, // Tidak dipakai di render
  // ArrowDownRight, // Tidak dipakai di render
  Wallet,
  CreditCard,
  Banknote,
  Users,
...
export const Reports = () => {
  // const { getItem } = useLocalStorage(); // <-- HAPUS
  ...
  // --- LOGIC BARU: FETCH DATA DARI LARAVEL ---
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Simpan Produk (Mapping field database ke frontend)
        const mappedProducts = prodRes.data.data.map((p: any) => ({
          ...p,
          purchasePrice: p.buy_price || p.purchasePrice || 0, // Mapping buy_price -> purchasePrice
          minStock: p.min_stock || p.minStock || 0
        }));
        setProducts(mappedProducts);
        setExpenses(expRes.data.data);

        // Set default date range (last 30 days) - LOGIC ASLI LU
  ...
  // --- SISA KODINGAN DI BAWAH INI 100% ORIGINAL (LOGIC KALKULASI & UI) ---
  ...
  const totalCOGS = filteredTransactions.reduce((sum, transaction) => {
    return sum + transaction.items.reduce((itemSum, item) => {
      // FIX LOGIC: Paksa konversi ID ke number/string biar match
  ...
  // const totalCapitalInInventory = totalStockValue; // Unused variable
  ...
  const dailySalesData = Object.values(dailySalesMap).map(day => ({
    ...
    const dateA = new Date(a.date); // Note: date format dd MMM might not parse correctly in Date(), but for sort we rely on key order if created sequentially or revisit logic if dates jump years.
    // Simple fix for sorting if needed, but original logic kept as requested.
    return 0;
  });
  ...
  const handleExport = () => {
    // 1. Header CSV (Pake titik koma biar aman di Excel Indo/US)
    ...
    const rows = filteredTransactions.map(t => {
      ...
      // Bersihin format biar CSV gak error (misal nama ada koma)
      const cleanData = [
        t.id,
        format(date, 'dd/MM/yyyy', { locale: id }),
        format(date, 'HH:mm', { locale: id }),
        `"${t.cashierName}"`, // Kasih tanda kutip jaga-jaga nama panjang
        t.paymentMethod === 'cash' ? 'Tunai' : t.paymentMethod === 'card' ? 'Kartu' : 'E-Wallet',
        t.total, // Angka murni tanpa "Rp" biar bisa dihitung di Excel
        itemCount
      ];
      return cleanData.join(';'); // Pake titik koma (;) sebagai pemisah
    });

    // 3. Gabungin Header + Data
    const csvContent = [headers.join(';'), ...rows].join('\n');

    // 4. Tambahin BOM (Byte Order Mark) biar Excel baca UTF-8 dengan bener
    const blob = new Blob(["\uFEFF" + csvContent], { type: 'text/csv;charset=utf-8;' });

    // 5. Download Trigger
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
    link.download = `Laporan_Transaksi_${format(new Date(), 'yyyy-MM-dd')}.csv`;
  ...
  return (
    <div className="space-y-6">
      {/* STYLE KHUSUS PRINT: Sembunyikan semua elemen layout pembungkus (Sidebar/Navbar) */}
      <style>{`
        @media print {
      ...
      {/* Bungkus konten utama dengan class print-area */}
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Popover open={datePickerOpen === 'start'} onOpenChange={(open: boolean) => setDatePickerOpen(open ? 'start' : null)}>
                ...
                <div className="space-y-2">
                  ...
                  <Popover open={datePickerOpen === 'end'} onOpenChange={(open: boolean) => setDatePickerOpen(open ? 'end' : null)}>
                    <PopoverTrigger asChild>
                      <Button
                        variant="outline"
                        className="w-full justify-start text-left border-2 h-11"
                      ...
                      return (
                        <div key={cashier.name} className="p-4 rounded-lg border-2 hover:border-primary/50 transition-colors">
                          <div className="flex items-start justify-between mb-3">
                            <div className="flex items-center gap-3">
                              <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-white ${index === 0 ? 'bg-yellow-500' : index === 1 ? 'bg-gray-400' : index === 2 ? 'bg-orange-600' : 'bg-primary'
                                }`}>
                                {index + 1}
                              </div>
                              <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <Card className="border-2 shadow-lg">
                ...
                <CardContent>
                  ...
                  <p className="text-xs text-muted-foreground mt-1">Margin {operatingExpenseRatio.toFixed(1)}%</p>
                </CardContent>
              </Card>
                ...
                <div className="space-y-4">
                  ...
                  <div className="border-l-4 border-orange-500 pl-4 space-y-3">
                    <div className="flex justify-between items-center">
                      ...
                      {/* Tampilin Angka Total */}
                      <p className="text-xl font-semibold text-orange-600">- {formatRupiah(operatingExpenses)}</p>
                    </div>

                    {/* Looping Rincian Pengeluaran */}
                    {filteredExpenses.length > 0 ? (
                      <div className="ml-2 space-y-1">
                        {filteredExpenses.map((exp, idx) => (
                          <div key={idx} className="flex justify-between text-xs text-muted-foreground">
                            {/* Format tanggal manual biar gak perlu import date-fns kalau ribet */}
                            <span>• {exp.description} ({new Date(exp.date).toLocaleDateString('id-ID', { day: 'numeric', month: 'numeric' })})</span>
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-xs text-primary/80 mt-1">
                          Margin Laba Bersih: {operatingExpenseRatio.toFixed(2)}%
                        </p>
                        <p className="text-xs text-muted-foreground mt-1">
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  <div className="p-4 bg-white rounded-lg shadow-sm border">
                    ...
                    <p className="text-2xl font-bold text-primary">{operatingExpenseRatio.toFixed(2)}%</p>
                  ...
                  <div className="p-4 bg-white rounded-lg shadow-sm border">
                    ...
                    <p className="text-2xl font-bold text-orange-600">{operatingExpenseRatio.toFixed(1)}%</p>
                      ...
                      <TableBody>
                        {topProducts.map((product, index) => (
                          <TableRow key={product.id}>
                            <TableCell>
                              <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-white ${index === 0 ? 'bg-yellow-500' : index === 1 ? 'bg-gray-400' : index === 2 ? 'bg-orange-600' : 'bg-primary'
                                }`}>
                                {index + 1}
                              </div>
                            </TableCell>
              ...
              <CardContent className="p-6">
                {(() => {
                  ...
                  const totalRestockValue = allLowStock.reduce((sum, product) => {
                    // @ts-ignore
                    const needed = Math.max((product.minStock || 0) - product.stock, 0) + 5; // Add buffer of 5 units
                    // @ts-ignore
                        ...
                        <Table>
                          ...
                          <TableBody>
                            {allLowStock.map((product, index) => {
                              ...
                              // @ts-ignore
                              const restockNeeded = Math.max((product.minStock || 0) - product.stock, 0) + 5;
                              // @ts-ignore
